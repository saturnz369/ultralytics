// Separate DeepStream prototype_v2 tracked-metadata control preview.
// This app computes virtual pan/tilt commands from DeepStream bbox metadata only.
...
main(int argc, char *argv[])
{
  ...
  pipeline = gst_pipeline_new("deepstream-yolo26-target-control");
  ...
  if (!pipeline || !source || !source_caps || !source_conv || !streammux || !pgie || !nvtracker || !nvvidconv || !nvosd || !sink) {
  ...
  gst_bin_add_many(GST_BIN(pipeline), source, source_caps, source_conv, streammux, pgie, nvtracker, nvvidconv, nvosd, sink, NULL);
  ...
  if (!gst_element_link_many(streammux, pgie, nvtracker, nvvidconv, nvosd, sink, NULL)) {
    g_printerr("Failed to link inference/tracker/display chain.\n");
    goto cleanup;
  }
  ...
  g_print("Starting prototype_v2 DeepStream target-control preview on CSI sensor %d\n", state.cfg.sensor_id);

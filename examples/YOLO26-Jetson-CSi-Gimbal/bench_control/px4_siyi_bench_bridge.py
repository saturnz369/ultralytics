...
# Use ardupilotmega here because it includes MOUNT_STATUS and GIMBAL_REPORT,
# which are useful for bench speed measurements.
...
class PX4SiyiBenchBridge:
    """Known-good bench-control bridge for Jetson -> PX4 -> SIYI tests."""
    ...
    def request_message(self, message_id: int, target_component: int | None = None) -> None:
        """Ask PX4/gimbal side to emit one MAVLink message if supported."""
        if self.master is None:
            return
        self._send_heartbeat()
        self.master.mav.command_long_send(
            self.args.mav_target_system,
            target_component if target_component is not None else mavutil.mavlink.MAV_COMP_ID_ALL,
    ...
    def recv_feedback(self, accepted_types: tuple[str, ...], timeout: float = 0.0):
        """Receive one feedback message from the accepted gimbal-related MAVLink types."""
        if self.master is None:
            return None
        return self.master.recv_match(type=list(accepted_types), blocking=timeout > 0.0, timeout=timeout if timeout > 0.0 else 0.0)

    def request_manager_status(self, timeout: float = 1.0) -> dict | None:
        """Request and return the current PX4 gimbal-manager status."""
        if self.master is None:
            return None
        self.request_message(281, target_component=mavutil.mavlink.MAV_COMP_ID_AUTOPILOT1)
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:

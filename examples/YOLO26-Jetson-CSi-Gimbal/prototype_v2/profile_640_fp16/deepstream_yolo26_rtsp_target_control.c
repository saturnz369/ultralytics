...
static gdouble
apply_feedforward_term(
    gdouble base_cmd,
    gdouble error_value,
    gdouble error_rate,
    gdouble feedforward_gain,
    const AppConfig *cfg)
{
  gdouble limit = fabs(cfg->max_command);
  gdouble activation_zone = fabs(cfg->feedforward_activation_zone);
  gdouble ff_limit = fabs(cfg->feedforward_limit);
  gdouble ff = 0.0;

  if (limit <= 1e-6) {
    return clamp_value(base_cmd, limit);
  }
  if (fabs(error_value) < activation_zone || fabs(error_rate) <= 1e-6) {
    return clamp_value(base_cmd, limit);
  }
  ...
  ff = clamp_value(fabs(error_rate) * fabs(feedforward_gain), ff_limit);
  return clamp_value(base_cmd + copysign(ff, error_value), limit);
}
                 ...
                 gdouble tilt_cmd)
{
   ...
   * Stage 1 T1 timestamp: this is the moment the DeepStream C app writes the
   * tracking/control metadata line. GLib returns monotonic time in
   * microseconds, so multiply by 1000 to store nanoseconds.
   *
   * Python reads this as ds_write_mono_ns and compares it with
   * time.monotonic_ns(). This is for same-machine latency measurement.
   * Remove only this block and the JSON field below if you want to undo.
   */
  const gint64 ds_write_mono_ns = g_get_monotonic_time() * 1000LL;
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    if (selected) {
      ...
      dy_norm = (selected->cy - (state->cfg.height / 2.0)) / (state->cfg.height / 2.0);
      {
        ...
        gdouble nominal_dt = (state->cfg.fps_n > 0)
                                ? (((gdouble) state->cfg.fps_d) / ((gdouble) state->cfg.fps_n))
                                : (1.0 / 30.0);
        gdouble dt_sec = nominal_dt;
        gdouble pan_error_rate = 0.0;
        gdouble tilt_error_rate = 0.0;
        gdouble ff_alpha = clamp_value(state->cfg.feedforward_alpha, 1.0);
        ...
        if (state->last_control_time_usec > 0) {
          dt_sec = ((gdouble) (now_usec - state->last_control_time_usec)) / 1000000.0;
          if (dt_sec < 1e-3 || dt_sec > 0.5) {
            dt_sec = nominal_dt;
          }
        }

        pan_error_rate = (pan_error - state->last_pan_error) / dt_sec;
        tilt_error_rate = (tilt_error - state->last_tilt_error) / dt_sec;

        state->smooth_pan_feedforward_rate =
            smooth_value(state->smooth_pan_feedforward_rate, pan_error_rate, ff_alpha);
        state->smooth_tilt_feedforward_rate =
            smooth_value(state->smooth_tilt_feedforward_rate, tilt_error_rate, ff_alpha);

        state->last_pan_error = pan_error;
        state->last_tilt_error = tilt_error;
        state->last_control_time_usec = now_usec;
      }
      ...
      pan_cmd = apply_feedforward_term(
          ...
          state->smooth_pan_feedforward_rate,
          state->cfg.pan_feedforward_gain,
          &state->cfg);
      tilt_cmd = apply_feedforward_term(
          tilt_cmd,
          state->smooth_tilt_error,
          state->smooth_tilt_feedforward_rate,
          state->cfg.tilt_feedforward_gain,
          &state->cfg);
      ...
      state->smooth_pan_feedforward_rate =
          smooth_value(state->smooth_pan_feedforward_rate, 0.0, state->cfg.feedforward_alpha);
      state->smooth_tilt_feedforward_rate =
          smooth_value(state->smooth_tilt_feedforward_rate, 0.0, state->cfg.feedforward_alpha);
      state->last_pan_error = 0.0;
      state->last_tilt_error = 0.0;
      state->last_control_time_usec = 0;
    }

    /* ===== LATENCY STAGE 2 ADDED START =====
     * Stage 2 control timestamp: target selection, error calculation, command
     * shaping and feedforward are done. Overlay/JSON writing happens after this.
     */

...
class PX4SiyiBridge:
    ...
    def __init__(self, args):
        ...
        self.last_manager_status_mono_ns: int | None = None
        self.last_device_attitude_status: dict | None = None
        self.last_device_attitude_status_mono_ns: int | None = None
        self.last_feedback_type: str | None = None
        self.last_feedback_mono_ns: int | None = None
        self.last_feedback_wall_time: float | None = None
        self.last_pitchyaw_send: float | None = None
        self._last_send_monotonic = 0.0
        self._last_heartbeat_monotonic = 0.0
        self._last_feedback_request_monotonic = 0.0
    ...
    def request_manager_status(self, timeout: float = 1.0) -> dict | None:
        ...
        if self.master is None:
            return None
        self._send_heartbeat(force=True)
        self._request_message(mavutil.mavlink.MAVLINK_MSG_ID_GIMBAL_MANAGER_STATUS)
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            ...
            self.last_manager_status_mono_ns = time.monotonic_ns()
            self.last_feedback_type = "GIMBAL_MANAGER_STATUS"
            self.last_feedback_mono_ns = self.last_manager_status_mono_ns
            self.last_feedback_wall_time = time.time()
            return status
        return None

    def _request_message(self, message_id: int) -> None:
        """Request one MAVLink message from PX4 without blocking."""
        if self.master is None:
            return
    ...
    def poll_feedback(self) -> None:
        """Non-blocking drain of gimbal/PX4 feedback messages."""
        if self.master is None or not self.connected or self.args.dry_run_mavlink:
            return

        now = time.monotonic()
        if (now - self._last_feedback_request_monotonic) >= 0.5:
            self._send_heartbeat(force=True)
            self._request_message(mavutil.mavlink.MAVLINK_MSG_ID_GIMBAL_MANAGER_STATUS)
            if hasattr(mavutil.mavlink, "MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS"):
                self._request_message(mavutil.mavlink.MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS)
            self._last_feedback_request_monotonic = now

        while True:
            msg = self.master.recv_match(
                type=["GIMBAL_MANAGER_STATUS", "GIMBAL_DEVICE_ATTITUDE_STATUS"],
                blocking=False,
            )
            if msg is None:
                break
            ...
            if msg_type == "GIMBAL_MANAGER_STATUS":
                self.last_manager_status = msg_dict
                self.last_manager_status_mono_ns = now_ns
            elif msg_type == "GIMBAL_DEVICE_ATTITUDE_STATUS":
                self.last_device_attitude_status = msg_dict
                self.last_device_attitude_status_mono_ns = now_ns

            self.last_feedback_type = msg_type
            self.last_feedback_mono_ns = now_ns
            self.last_feedback_wall_time = now_wall
